...
status: draft
confidence: medium
source_ids: [SRC-004, SRC-008, SRC-009, SRC-010, SRC-011, SRC-012, SRC-013, SRC-014, SRC-015, SRC-016]
...
5. 没有同构公开多 GPU comparator 时，使用 28/22 配对消融、绝对扩展数据和 P=1 外部锚点，并公开不可比原因；
6. 禁止使用“首次并行五对角”“首次分布式五对角”“首个任意带宽 GPU 求解器”和未经检索更新的“最优”。

## 原因

并行五对角、分区缩约、通用带状域分解和单 GPU batched pentadiagonal 均已有公开先例；但现有公开路线没有与本项目的跨 rank 批量线系统完全同构。cuSPARSE 是目标 NVIDIA 环境中最稳定、可获得且算法独立的外部锚点。
