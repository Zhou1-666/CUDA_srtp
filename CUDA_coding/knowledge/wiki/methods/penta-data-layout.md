...
| `BIGbuf_A/B` | 1D | forward/backward 复用的 device 通信缓冲 | plan |
...
其中 `D` 由 `pascalunpack_penta` 填为 1。

## 描述符

- forward gather：输入子块 `rank_line_count × 28`，目标完整块 `tmp_N × 32`；
